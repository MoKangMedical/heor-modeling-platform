...
class AnalysisTemplate(Base):
    ...
    model_version_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("model_versions.id", ondelete="CASCADE"), nullable=False
    )
    analysis_type: Mapped[str] = mapped_column(Text, nullable=False)
    name: Mapped[str] = mapped_column(Text, nullable=False)
    config_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    allowed_overrides_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
...
class RunMetricValue(Base):
    ...
    metric_catalog_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("run_metric_catalog.id", ondelete="CASCADE"), nullable=False
    )
...
class CalibrationConfig(Base):
    ...
    name: Mapped[str] = mapped_column(Text, nullable=False)
    target_series_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("clinical_series.id"), nullable=False
    )
...
class CalibrationParameter(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    calibration_config_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("calibration_configs.id", ondelete="CASCADE"), nullable=False
    )
...
class CalibrationResult(Base):
    ...
    calibration_config_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("calibration_configs.id"), nullable=False
    )
    ...
    best_params_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    diagnostics_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    overlay_artifact_id: Mapped[UUID | None] = mapped_column(PGUUID(as_uuid=True), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class PatientStateEvent(Base):
    ...
    tracker_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class CohortAggregate(Base):
    ...
    metric_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
