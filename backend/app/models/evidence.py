...
class ClinicalSeriesPoint(Base):
    ...
    series_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("clinical_series.id", ondelete="CASCADE"), nullable=False
    )
...
class CurveFit(Base):
    ...
    fit_stats_json: Mapped[dict] = mapped_column(JSONB, nullable=False, default=dict)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
...
class CompoundCurveSegment(Base):
    ...
    id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    compound_curve_id: Mapped[UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("compound_curves.id", ondelete="CASCADE"), nullable=False
    )
    seq_no: Mapped[int] = mapped_column(Integer, nullable=False)
    segment_kind: Mapped[str] = mapped_column(Text, nullable=False)
    source_ref_id: Mapped[UUID] = mapped_column(PGUUID(as_uuid=True), nullable=False)
